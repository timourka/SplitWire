...
try {
    ...
    $env:GOARCH = 'amd64'
    $env:CGO_ENABLED = '0'
    & $goExe build -buildvcs=false -trimpath -ldflags '-s -w -H=windowsgui' -o dist\SplitWire.exe .\cmd\splitwire
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
} finally {
    $env:GOOS = $oldGOOS
...
Copy-Item -Force .\CHANGELOG_1.2.2.md .\dist\CHANGELOG_1.2.2.md
